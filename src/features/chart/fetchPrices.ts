...
import { NativeTokenId, NativeTokens, StableTokenIds } from 'src/config/tokens'
import {
...
>('chart/fetchPrices', async (params, thunkAPI) => {
  const { baseCurrency, numDays } = params
  const prices: BaseCurrencyPriceHistory = thunkAPI.getState().tokenPrice.prices
  const pairPriceUpdates = await _fetchTokenPrice(prices, baseCurrency, numDays)
...
async function _fetchTokenPrice(
  ...
  const pairPriceUpdates = await fetchStableTokenPrices(numDays, prices[baseCurrency])
  return pairPriceUpdates
}
